...
int main(int argc, char **argv) 
{
	...
	{
		printf("%s <broadcast address> <port number (range: 1025 - 65535)>\n", argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	broadcast_enable = 1;
	ret = setsockopt(udp_sockfd, SOL_SOCKET, SO_BROADCAST, &broadcast_enable, sizeof(broadcast_enable));
	if(ret < 0)
		handle_error("setsockopt broadcast enable");
	...
	while (1)
	{
		...
		printf("Sended date string to broadcast: %s\n", date);
		sleep(2);
