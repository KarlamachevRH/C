 ...
 * Return 0 on success.
 */
int read_arp(int fd, unsigned char *sender_mac)
{
    debug("read_arp");
    int ret = -1;
    unsigned char buffer[BUF_SIZE];
    ...
    debug("Destination IP: %s", inet_ntoa(sender_a));
    ...
    memmove(sender_mac, arp_resp->sender_mac, MAC_LENGTH);
    ret = 0;
...
void get_dest_mac(const char *ifname, in_addr_t *ip, unsigned char *sender_mac)
{
    ...
    while(1) 
    {
        int r = read_arp(arp_fd, sender_mac);
        if (r == 0)
        {
